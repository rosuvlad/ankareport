...
describe("getItems", () => {
  ...
  describe("orderBy with $. support", () => {
    test("sorts primitive strings ASC using $.", () => {
      const layout: ILayout = {
        width: 500,
        headerSection: { height: 0, binding: "", items: [] },
        contentSection: {
          ...
          orderBy: "$.",
          items: [
            { type: "text", x: 0, y: 0, width: 100, height: 20, name: "", text: "", binding: "$." },
          ],
        },
        footerSection: { height: 0, binding: "", items: [] },
      };
      ...
      expect(textItems[0].text).toBe("a");
      expect(textItems[1].text).toBe("b");
      expect(textItems[2].text).toBe("c");
    });

    test("sorts primitive numbers DESC using $. DESC", () => {
      const layout: ILayout = {
        width: 500,
        headerSection: { height: 0, binding: "", items: [] },
        contentSection: {
      ...
      const textItems = items.filter(x => x.type === "text");
      expect(textItems.length).toBe(3);
      expect(textItems[0].text).toBe("3");
      expect(textItems[1].text).toBe("2");
      expect(textItems[2].text).toBe("1");
    });
  });

  test("sorts with explicit $. prefix and multiple fields", () => {
    const layout: ILayout = {
      width: 500,
      headerSection: { height: 0, binding: "", items: [] },
      contentSection: {
        height: 25,
        binding: "$.items",
        orderBy: "$.group DESC, $.value ASC",
        items: [
          { type: "text", x: 0, y: 0, width: 100, height: 20, name: "", text: "", binding: "$.value" },
        ],
      },
      footerSection: { height: 0, binding: "", items: [] },
    };
    ...
    expect(textItems[0].text).toBe("1"); // B, 1
    expect(textItems[1].text).toBe("4"); // B, 4
    expect(textItems[2].text).toBe("2"); // A, 2
    expect(textItems[3].text).toBe("3"); // A, 3
  });
});
